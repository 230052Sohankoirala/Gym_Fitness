...
export default function TrainerClient() {
    ...
    // --- state ---
    const [clients, setClients] = useState([]); // [{ user, sessions: [] }]
    const [loading, setLoading] = useState(true);
    const [errMsg, setErrMsg] = useState("");

    // --- axios instance ---
    const api = useMemo(() => {
        const token = localStorage.getItem("trainerToken") || localStorage.getItem("token");
        return axios.create({
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                // ✅ NEW endpoint that returns sessions grouped by client
    ...
    // ---- helpers ----
    const toDisplay = (v) => (v === null || v === undefined || v === "" ? "N/A" : String(v));

    const pick = (obj, keys) => {
        for (const k of keys) {
            const val = k.split(".").reduce((acc, part) => (acc ? acc[part] : undefined), obj);
    ...
    const formatGoals = (g) => {
        ...
        if (Array.isArray(g)) return g.filter(Boolean).join(", ") || "N/A";
        if (typeof g === "object") {
    ...
    const statusPill = (s) => {
        ...
        if (st === "confirmed" || st === "booked") return darkMode ? "bg-green-900/40 text-green-200" : "bg-green-100 text-green-700";
        if (st === "pending") return darkMode ? "bg-yellow-900/40 text-yellow-200" : "bg-yellow-100 text-yellow-700";
        if (st === "cancelled") return darkMode ? "bg-red-900/40 text-red-200" : "bg-red-100 text-red-700";
        if (st === "completed") return darkMode ? "bg-blue-900/40 text-blue-200" : "bg-blue-100 text-blue-700";
        if (st === "inprogress") return darkMode ? "bg-purple-900/40 text-purple-200" : "bg-purple-100 text-purple-700";
        ...
        return (
            <div className={`min-h-screen ${baseBg} ${text} flex items-center justify-center`}>
                <div className={`rounded-xl border ${border} ${cardBg} p-6`}>
                    ...
                    <p className={`mt-2 text-sm ${muted}`}>Check your backend route and trainer token.</p>
                </div>
            </div>
        );
    }
                ...
                <main className="mx-auto max-w-6xl p-6">
                    <div className="flex items-end justify-between gap-3 mb-6">
                        <div>
                            ...
                            <p className={`text-sm ${muted}`}>Each client may appear with multiple joined sessions.</p>
                        </div>
                    ...
                    {clients.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            {clients.map((row, idx) => {
                                ...
                                const rawGoals = pick(u, ["goals", "goal", "userGoals", "profile.goals", "fitnessGoals"]);
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-start justify-between gap-3 mb-2">
                                            ...
                                            <span className={`px-3 py-1 text-xs rounded-full font-medium ${getBadgeColor(membership)}`}>
                                                {membership}
                                            </span>
                                        </div>
                                        ...
                                        <div className="mt-3 space-y-1 text-sm">
                                            <p>
                                                <span className="font-medium">Weight:</span> {toDisplay(weight)}
                                            </p>
                                            <p>
                                                <span className="font-medium">Height:</span> {toDisplay(height)}
                                            </p>
                                            <p>
                                                <span className="font-medium">Age:</span> {toDisplay(rawAge)}
                                            </p>
                                            <p>
                                                <span className="font-medium">Goal:</span> {formatGoals(rawGoals)}
                                            </p>
                                        </div>
                                        ...
                                        <div className="mt-5">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <span className={`text-xs ${muted}`}>{sessions.length} joined</span>
                                            </div>

                                            {sessions.length ? (
                                                <div className={`rounded-xl border ${border} ${softBg} p-2 max-h-44 overflow-y-auto`}>
                                                    <div className="space-y-2">
                                                        {sessions.map((s, i) => {
                                                            ...
                                                            return (
                                                                <div key={sid} className={`rounded-lg border ${border} p-2 ${darkMode ? "bg-gray-800" : "bg-white"}`}>
                                                                    <div className="flex items-start justify-between gap-3">
                                                                        ...
                                                                        <div className="text-right shrink-0">
                                                                            ...
                                                                            <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${statusPill(status)}`}>
                                                                                {status}
                                                                            </span>
                                                                        </div>
                                                                    </div>
                                                ...
                                                <div className={`rounded-xl border ${border} ${softBg} p-3`}>
                                                    <p className={`text-xs ${muted}`}>No session joins found for this client.</p>
                                                </div>
                                            )}
                                        </div>

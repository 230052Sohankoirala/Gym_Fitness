...
const normalizeUrl = (u) => {
  ...
  if (s.startsWith("/")) return `${SOCKET_BASE}${s}`;
...
export default function MessagePortal() {
  ...
  const [trainerInfo, setTrainerInfo] = useState({ name: "Trainer", email: "" });
  ...
  const fetchTrainerInfo = useCallback(async () => {
    ...
    try {
      ...
      if (!data) throw new Error("Trainer route not found");

      const name =
        data?.name || data?.fullName || data?.fullname || data?.trainerName || "Trainer";

      setTrainerInfo({ name, email: data?.email || "" });
    } catch {
      setTrainerInfo({ name: "Trainer", email: "" });
    } finally {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      socket.emit("chat:join", { trainerId });
    });
  ...
  const sendText = useCallback(async () => {
    ...
    try {
      socketRef.current?.emit("chat:send", { trainerId, text: clean });
      setText("");
      setTimeout(() => scrollBottom(true), 20);
    } catch {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className={`text-sm truncate ${softText}`}>
                  {trainerInfo?.email || "Messages are saved and synced with your trainer"}
                </p>
              </div>
            ...
            {loading ? (
              ...
              <div className="h-full flex items-center justify-center">
                <div className="max-w-sm text-center">
                  ...
                  <p className={`text-sm ${softText}`}>
                    Say hello to your trainer. Your messages will stay here and load every time you return.
                  </p>
                </div>
              </div>
            ) : (
              <AnimatePresence initial={false}>
                {renderedItems.map((item, idx) => {
                  ...
                  const showMeta = !prev || prevIsTrainer !== isTrainer || !sameDay(prev?.createdAt, m?.createdAt);
          ...
          >
            ...
            <div className="flex items-end gap-2 sm:gap-3">
              ...
              <div className="flex-1">
                ...
                />
                <div className={`mt-2 text-[11px] flex items-center justify-between ${softText}`}>
              ...
              <motion.button
                whileHover={{
                  scale:
                    sending || (!text.trim() && files.length === 0) || socketState !== "online"
                      ? 1
                      : 1.04,
                }}
                whileTap={{

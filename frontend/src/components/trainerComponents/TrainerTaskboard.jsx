...
/* ================== Axios instance (adds Bearer token) ================== */
const api = axios.create({ baseURL: "http://localhost:4000" });
api.interceptors.request.use((config) => {
    const token =
        localStorage.getItem("token") || localStorage.getItem("trainerToken"); // fallback just in case
    if (token) config.headers.Authorization = `Bearer ${token}`;
    return config;
});

/* ================== Kanban Columns (match backend keys) ================== */
// replace your columns array
const columns = [
...
const TrainerTaskBoard = () => {
    ...
    useEffect(() => {
        const fetchTasks = async () => {
            try {
                ...
                console.error("Error fetching trainer tasks:", err.response?.status, err.response?.data || err.message);
    ...
    const addTask = async () => {
        ...
        if (!title) {
            setErrors({ todo: "Task title is required" });
            return;
        }
        try {
            const { data } = await api.post("/api/trainers/tasks", { title, column: "todo" });
            setTasks((prev) => [...prev, data]);
            setTaskInputs({ todo: { show: false, title: "" } });
            setErrors({});
            setErrMsg("");
        } catch (err) {
            console.error("Error adding trainer task:", err.response?.status, err.response?.data || err.message);
    ...
    const moveTask = async (task, newCol) => {
        try {
            const { data } = await api.put(`/api/trainers/tasks/${task._id}`, { column: newCol });
            setTasks((prev) => prev.map((t) => (t._id === data._id ? data : t)));
            setErrMsg("");
        } catch (err) {
            console.error("Error moving trainer task:", err.response?.status, err.response?.data || err.message);
    ...
    const deleteTask = async (taskId) => {
        try {
            ...
            console.error("Error deleting trainer task:", err.response?.status, err.response?.data || err.message);
        ...
        <div className="mt-12">
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {columns.map((col) => (
                    <div
                        key={col.key}
                        className={`w-full rounded-xl shadow-sm p-4 border transition-colors duration-200
                                ...
                                {tasks
                                    ...
                                    .map((task) => (
                                        <Reorder.Item
                                            ...
                                            className={`p-3 rounded-lg shadow-sm cursor-grab transition-colors duration-200 ${col.color} ${darkMode ? "text-gray-200" : "text-white"
                                                }`}
                                            whileDrag={{ scale: 1.05 }}
                                        >
                                            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                                                <span className="text-sm md:text-base">{task.title}</span>
                            ...
                            <>
                                {taskInputs.todo.show ? (
                                    <div className="mt-2 flex flex-col gap-2">
                                        <input
                                            type="text"
                                            placeholder={errors.todo || "Enter task"}
                                            value={taskInputs.todo.title}
                                            onChange={(e) =>
                                                setTaskInputs({ todo: { ...taskInputs.todo, title: e.target.value } })
                                            }
                                    ...
                                    <button
                                        ...
                                        onClick={() => setTaskInputs({ todo: { ...taskInputs.todo, show: true } })}
                                    >
